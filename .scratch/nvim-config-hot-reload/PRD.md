...
Closing as `wontfix`. Session restore (`<leader>QS`) has made full restarts
cheap enough that this isn't worth building. It's also only a partial gap
in practice: `lua/plugins/*.lua` reload already exists for free via
lazy.nvim's built-in `:Lazy reload <plugin>` command — the exact case that
motivated this (editing `agent.lua`/`telescope.lua`) is solvable today with
no new code. Only `lua/config/*.lua` (options/keymaps/buffers/window-dim)
has no reload path, and closing the gap there isn't worth it given restart
is now cheap. Full reasoning recorded at
`.out-of-scope/config-hot-reload.md`.
